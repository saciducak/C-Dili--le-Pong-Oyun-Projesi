...
void RandomBlockGenerator() {
	for (unsigned int CreatedBlock = 0; CreatedBlock < MAXIMUMACTIVEBLOCKCOUNT - CountOfActiveBlocks; CreatedBlock++) {
		...
		float GridSizeY = (float)((float)(SCREENHEIGHT - 225.0) / ((float)(BlockHeight + 3)));
		unsigned char isFoundAnEmptyPlace = 0;
		while (isFoundAnEmptyPlace == 0) {
			float PosX = ((rand() % (unsigned int)(GridSizeX)) * (BlockWidth + 3)) + 10.0f;
			float PosY = ((rand() % (unsigned int)(GridSizeY)) * (BlockHeight + 3)) + 125.0f;
...
//hizlar
float ballsSpeed = 7.0f, paddlesSpeed = 17.0f, ballsFriction = 1.01f, paddelsBouncePower = 1.65f;

//bloklarin boyutlari
int arkaplanX = SCREENWIDTH, arkaplanY = SCREENHEIGHT, paddleX = 90, paddleY = 9, topXY = 13;

//puan sistemi degiskenleri
int p1_asil_puan = 0, p1_carpan_puan = 0, p1_topbizde = 1, p2_asil_puan = 0, p2_carpan_puan = 0, p2_topbizde = 0, limit_deger = 50;
...
float saniye0 = 0, saniye1 = 0, saniye2 = 0, saniye3 = 0;
int sayabilirsin0 = 0, sayabilirsin1 = 0, sayabilirsin2 = 0, sayabilirsin3 = 0, p1_buyudu = 0, p1_kuculdu = 0;
...
void Setup() {
	...
	puan1sprite->WIDTH = 50;
	puan1sprite->HEIGHT = 20;
	puan1sprite->PosX = 15;
	puan1sprite->PosY = 10;
	puan2sprite = CreateSprite(puantexture);
	puan2sprite->WIDTH = 50;
	puan2sprite->HEIGHT = 20;
	puan2sprite->PosX = 15;
	...
	pad.posy = 550;
	pad.posx = paddlesSpeed * 5 + 7;
	pad2.posy = 50;
	pad2.posx = paddlesSpeed * 5 + 7;
...
void GameLoop(unsigned char isTwoPlayer) {
	...
	if (CollideTopwithTexture(topsprite, &top, p2_paddlesprite)) {
		...
		p2_topbizde = 1;
		p1_topbizde = 0;
		if (IsKey_Pressed(KEYBOARD_NP_4)) {
			if (top.vx > 0) {
				top.vx = top.vx / paddelsBouncePower;
				top.vy = -top.vy;
				RandomBlockGenerator();
		...
		else if (IsKey_Pressed(KEYBOARD_NP_6)) {
			if (top.vx > 0) {
				top.vx = top.vx * paddelsBouncePower;
				top.vy = -top.vy;
				RandomBlockGenerator();
	...
	if (top.isMoving == 0 && (IsKey_Pressed(KEYBOARD_A) || IsKey_Pressed(KEYBOARD_D) || IsKey_Pressed(KEYBOARD_NP_4) || IsKey_Pressed(KEYBOARD_NP_6))) {
		top.isMoving = 1;
	}
	if (top.isMoving) {
		top.posx += top.vx;
	...
	//Player 1 Skor Tablosunu Goster
	{
		...
		for (unsigned char i = 0; i < AsilPuanlog10; i++) {
			...
			AsilPuanSprite[AsilPuanlog10 - i - 1]->WIDTH = 15;
			AsilPuanSprite[AsilPuanlog10 - i - 1]->HEIGHT = 20;
		...
		for (unsigned char i = 0; i < CarpanPuanlog10; i++) {
			...
			CarpanPuanSprite[CarpanPuanlog10 - i - 1]->WIDTH = 15;
			CarpanPuanSprite[CarpanPuanlog10 - i - 1]->HEIGHT = 20;
